...
const useColumnVisibility = (columns = [], options = {}) => {
  ...
  const initializeDefaultVisibility = useCallback(() => {
    ...
    return visibility;
  }, [flatColumns, defaultHiddenColumns, columns]);
  ...
  const toggleColumnVisibility = useCallback((columnId) => {
    ...
    // 그룹 컬럼인지 확인
    const groupColumn = columns.find(col => col.id === columnId && col.type === 'group');
  ...
  }, [alwaysVisibleColumns, columns, columnVisibility]);
