    ...
    switch (filterId) {
      ...
      case 'date':
        ...
        if (filterValue.startDate) {
          dateFilteredResult = dateFilteredResult.filter(item => item.id >= 1010);
        }
        
        if (filterValue.endDate) {
          dateFilteredResult = dateFilteredResult.filter(item => item.id <= 1040);
        }
  ...
  return (
    <PageContainer>
      ...
      <PageHeader
        ...
        onRefreshClick={() => alert('입금신청 목록 새로고침')}
        sx={{ mb: 2 }}
      />
